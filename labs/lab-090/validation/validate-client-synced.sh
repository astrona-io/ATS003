...
selected="$(echo "$sources" | grep -E '^\^\*.*astrona-ats-003-lab-090-server' || true)"

if [[ -z "$selected" ]]; then
  echo "FAIL: client-synced - astrona-ats-003-lab-090-server is not the currently selected (^*) source; got: $sources"
  exit 1
fi
...
if [[ "$leap" == "Normal" ]] && [[ "$stratum" =~ ^[0-9]+$ ]] && (( stratum >= 2 && stratum <= 15 )); then
  echo "PASS: client-synced - synced via astrona-ats-003-lab-090-server, Leap status Normal, Stratum $stratum"
  exit 0
else
