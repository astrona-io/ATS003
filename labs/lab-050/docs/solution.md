...
192.168.10.71 astro-ats-003-lab-050
EOF
```

Check `man 5 hosts` — the file format is `IP_address canonical_hostname [aliases...]`, one mapping per line. This single line serves both directions: forward lookups of `astro-ats-003-lab-050` return `192.168.10.71`, and reverse lookups of `192.168.10.71` return `astro-ats-003-lab-050`, because glibc's `files` NSS backend scans `/etc/hosts` in both directions from the same table — there's no separate "reverse zone" file needed the way DNS requires a PTR record.
...
```bash
getent hosts astro-ats-003-lab-050
```
...
# Netplan path:
sudo $EDITOR /etc/netplan/50-astro-ats-003-lab-050-secondary.yaml
sudo netplan generate
sudo netplan apply
